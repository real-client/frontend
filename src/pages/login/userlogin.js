...
function Login() {
  ...
  const handleEmailChange = (event) => {
    ...
    setInputsFilled((prev) => ({
      ...prev,
      active_email: Boolean(event.target.value.trim()),
    }));
  };
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:5000/user/login", { active_email, password })
      .then((response) => {
        const data = response.data;
        if (data && data.token) {
          // Successful login
          console.log(response);
          localStorage.setItem("email", active_email || "");
          ...
          <form className="login-form">
            ...
            <div
              className={`email-wrapper ${
                active_email || inputsFilled.active_email ? "has-value" : ""
              }`}
            >
              <input
                type="email"
              ...
              <label
                className={`email ${
                  active_email || inputsFilled.active_email
                    ? "input-filled"
                    : ""
                }`}
                htmlFor="email"
